...
non-digit character or the end of the input is reached. The rest of the string is ignored. Convert these digits into
    ...
    def myAtoi(self, s: str) -> int:
        ...
        if s:
            if len(s) == 1:
                ...
                for i, letter in enumerate(s[from_index+1:]):
                    if letter.isdigit():
                        to_index = to_index + 1
                    else:
                        break
                result = int(s[from_index:to_index+1]) * sign
                if result < -(2**31):
                    result = -(2**31)
                elif result > (2**31)-1:
                    result = (2**31)-1
